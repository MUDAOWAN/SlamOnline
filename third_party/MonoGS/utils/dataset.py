...
class TUMParser:
    ...
    def load_poses(self, datapath, frame_rate=-1):
        # Original logic required a TUM groundtruth/pose file:
        # if os.path.isfile(os.path.join(datapath, "groundtruth.txt")):
        #     pose_list = os.path.join(datapath, "groundtruth.txt")
        # elif os.path.isfile(os.path.join(datapath, "pose.txt")):
        #     pose_list = os.path.join(datapath, "pose.txt")
        #
        # For offline RealSense RGB-D captures, groundtruth poses may not exist.
        # In that case we associate rgb/depth only and use identity poses as
        # initialization, matching the live RealSense dataset behavior.
        pose_list = None
        if os.path.isfile(os.path.join(datapath, "groundtruth.txt")):
            pose_list = os.path.join(datapath, "groundtruth.txt")
        elif os.path.isfile(os.path.join(datapath, "pose.txt")):
        ...
        if pose_list is None:
            print(f"[warn] No groundtruth.txt or pose.txt found in {datapath}; using identity poses for RGB-D SLAM initialization.")
            pose_vecs = None
            tstamp_pose = None
        else:
